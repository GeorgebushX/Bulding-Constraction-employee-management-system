...
// // GET - Get contractors by worker role with site details
// export const getContractorsByWorkerRole = async (req, res) => {
//   try {
//     const { workerRole } = req.params;
...
// // POST - Create a new worker with contractorName and site derived from contractor
// export const createWorker = async (req, res) => {
//   try {
//     const {
//       name, email, gender, phone, alternatePhone, address,
...
//     // Find contractor by name and role with populated site
//     const contractor = await Contractor.findOne({ 
//       name: contractorName,
//       contractorRole: ROLE_MAPPING[workerRole]
//     }).populate('site', 'name');
...
//     // Get site from contractor
//     const site = contractor.site;
//     if (!site) {
//       return res.status(400).json({
//         success: false,
//         message: "Contractor is not assigned to any site"
//       });
//     }
...
//     // Verify references exist before population
//     const [contractorExists, siteExists] = await Promise.all([
//       Contractor.exists({ _id: newWorker.contractorId }),
//       Site.exists({ _id: newWorker.site })
//     ]);

//     if (!contractorExists || !siteExists) {
//       console.warn('Reference check failed:', {
//         contractorExists,
//         siteExists,
//         contractorId: newWorker.contractorId,
//         siteId: newWorker.site
//       });
//     }

//     // Get the populated worker data with explicit model references
//     const result = await Worker.findOne({ _id: newWorker._id })
//       .populate({
//         path: 'contractorId',
//         model: 'Contractor',
...
// // GET - Get all workers with site and contractor details
// export const getAllWorkers = async (req, res) => {
//   try {
//     const { search = '', siteName, contractorName, role } = req.query;
...
//     if (siteName) {
//       query['site.name'] = siteName;
//     }
    
//     if (contractorName) {
//       const contractorDoc = await Contractor.findOne({ name: contractorName });
//       if (contractorDoc) {
//         query.contractorId = contractorDoc._id;
//       } else {
//         return res.status(404).json({
...
//       .populate({
//         path: 'contractorId',
//         select: 'name contractorRole site',
//         populate: {
//           path: 'site',
//           select: 'name'
//         }
...
//     // Map the results to include siteName from contractor if not directly available
//     const formattedWorkers = workers.map(worker => ({
//       ...worker,
//       siteName: worker.site?.name || worker.contractorId?.site?.name
//     }));
...
// // GET - Get worker by ID with site and contractor details
// export const getWorkerById = async (req, res) => {
//   const { id } = req.params;
//   try {
//     let worker;
...
//     // Add siteName from contractor if not directly available
//     const formattedWorker = {
//       ...worker,
//       siteName: worker.site?.name || worker.contractorId?.site?.name
//     };
...
//       const workerRole = updateData.workerRole || worker.workerRole;
//       const contractor = await Contractor.findOne({ 
//         name: updateData.contractorName,
//         contractorRole: ROLE_MAPPING[workerRole]
//       }).populate('site');
...
//       worker.contractorId = contractor._id;
//       worker.site = contractor.site?._id || null;
//     }
...
//     // Update worker sub-role based on workerRole
//     if (updateData.workerRole && updateData.workerSubRole) {
//       worker.workerSubRole = updateData.workerSubRole;
//     }
...
//     const formattedWorker = {
//       ...updatedWorker,
//       siteName: updatedWorker.site?.name || updatedWorker.contractorId?.site?.name
//     };
...
//     if (worker.workerIdProof?.length > 0) {
//       worker.workerIdProof.forEach(proof => {
//         const proofPath = path.join(process.cwd(), 'public', proof);
//         if (fs.existsSync(proofPath)) {
//           fs.unlinkSync(proofPath);
//         }
